			...
			<div id="board">
								...
								<tr>
									...
									<td style='padding-left:${15*vo.depth }px;text-align:left;'>
										...
										<c:if test="${vo.statusNo eq 0 || vo.statusNo eq 1 }">
											<a href="${pageContext.servletContext.contextPath }/board?a=view&no=${vo.no}&page=${paging.curPageNum }">${vo.title }</a>
										</c:if>
									</td>
									<td>${vo.userName }</td>
									<td>${vo.hit }</td>
									<td>${vo.regDate }</td>
									<td><c:if test="${authUser.no eq vo.userNo }"><a href="${pageContext.servletContext.contextPath }/board?a=delete&no=${vo.no}&gNo=${vo.gNo }&page=${paging.curPageNum }" class="del">삭제</a></c:if></td>
								</tr>
							</c:forEach>
						</table>
				<!-- pager 추가 -->
				...
				<div class="bottom">
					<c:if test="${!empty authUser.no }">
						<a href="${pageContext.servletContext.contextPath }/board?a=writeform&page=${paging.curPageNum }" id="new-book">글쓰기</a>
					</c:if>
				</div>				
			</div>
		</div>
